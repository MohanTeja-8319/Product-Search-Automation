  ...
  return (
    <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-xl mt-6 p-4 sm:p-6">
      ...
      <div className="overflow-x-auto">
        <div className="flex items-end justify-start sm:justify-around gap-6 sm:gap-0 min-w-max sm:min-w-0 h-64 sm:h-72 border-b border-l border-gray-300 pb-4 px-2">
          {comparison.map((item) => {
            const height =
              ((item.price - lowest) / (highest - lowest || 1)) * 150 + 40;
            ...
            return (
              <div key={item.id} className="flex flex-col items-center">
                ...
                <div
                  className={`w-10 sm:w-14 rounded-t-lg transition-all duration-300 hover:scale-105 ${
                    item.price === lowest ? "bg-green-500" : "bg-purple-600"
                  }`}
                  style={{ height: `${height}px` }}
                ></div>

                <div className="mt-3 text-xs sm:text-sm font-medium whitespace-nowrap">
                  {item.store}
                </div>
              </div>
            );
