      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
        {deals.map((item) => (
          ...
          >
            ...
            <button className="absolute top-4 right-4 text-gray-400 hover:text-red-500 hover:bg-gray-50 p-1.5 rounded-full transition duration-200 z-10">
              <FiHeart className="text-sm" />
            </button>

            {/* Product Image Area */}
            <div className="w-full h-32 flex items-center justify-center bg-gray-50 rounded-xl mb-3 overflow-hidden p-3 select-none">
              <img
                src={item.image}
                alt={item.name}
            ...
            <div className="text-center mb-3">
              <h3 className="font-bold text-xs text-gray-900 leading-tight line-clamp-1">{item.name}</h3>
              <p className="text-[10px] font-semibold text-gray-400 mt-0.5">{item.category}</p>
            </div>

            {/* Price & Drop Row */}
            <div className="flex justify-between items-center px-1 mb-3">
              <span className="font-extrabold text-sm text-gray-950">{item.price}</span>
              ...
              {item.stores.map((store, idx) => (
                <div key={idx} className="flex justify-between items-center text-[10px] px-0.5">
                  <div className="flex items-center gap-1.5 text-gray-500 font-semibold">
                    <span className={`w-3.5 h-3.5 rounded flex items-center justify-center font-bold text-[8px] uppercase select-none ${store.logoBg}`}>
                      {store.logo}
                    </span>
            ...
            {/* Action Button */}
            <button className="w-full mt-2 py-2 bg-indigo-50/50 hover:bg-indigo-50 border border-indigo-100/30 text-indigo-600 text-xs font-bold rounded-xl transition duration-200">
              Compare Prices
            </button>
          </div>
        ))}
